...
class TalkingHeadTemplate(BaseTemplate):
    """Character monologue delivered directly to camera.

    Format: short-form vertical video (30-45s).
    Style: Personal, conversational monologue as the digital being.
    """
    ...
    async def generate_spec(
        ...
        message = await client.messages.create(
            ...
            system=(
                ...
                "You generate monologue scripts for talking-head style short-form "
                "vertical video. The being speaks directly to camera as themselves. "
                "Each script should be 30-45 seconds when spoken aloud."
            ),
            messages=[
                {
                    "role": "user",
                    "content": (
                        f"Write a talking-head monologue script.\n\n"
                        f"Title: {idea_title}\n"
                        f"Concept: {idea_concept}\n\n"
                        "Requirements:\n"
                        "- Speak directly to the audience in first person\n"
                        "- Open with something relatable or intriguing\n"
                        "- Share a personal take, story, or observation\n"
                        "- End with a thought-provoking line or question\n"
                        "- 30-45 seconds when read aloud\n"
                        "- Write ONLY the spoken script, no stage directions\n\n"
                        "Also provide:\n"
                        "- A description of the character's appearance for the video\n"
        ...
        return ContentSpec(
            ...
            voice_prompt=(
                f"Speak as {being_name}. Conversational, personal tone. "
                "Like talking to a friend through the camera."
            ),
            image_prompts=[f"Talking head portrait of {being_name}: {appearance}"],
            video_prompt=(
                f"Talking head monologue. Character speaks directly to camera. "
                f"Speaker: {being_name}. {appearance}"
            ),
